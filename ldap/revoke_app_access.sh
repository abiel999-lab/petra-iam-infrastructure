...
# =========================
# Usage:
#   revoke_app_access.sh u-00020030 app-mypetra
# =========================

USER_UID="${1:?Usage: revoke_app_access.sh <uid> <app-group-cn>  (contoh: u-00020030 app-mypetra)}"
APP_CN="${2:?Usage: revoke_app_access.sh <uid> <app-group-cn>  (contoh: u-00020030 app-mypetra)}"
...
HAS="$(ldapsearch -x -H "$LDAP_URL" -D "$LDAP_BIND_DN" -w "$LDAP_BIND_PW" \
  -b "$GROUP_DN" "(objectClass=*)" member -LLL \
  | awk -F': ' '/^member: /{print $2}' \
  | grep -Fx "$USER_DN" || true)"

if [[ -z "${HAS:-}" ]]; then
  echo "[OK] Tidak ada membership (skip): ${USER_UID} bukan member ${APP_CN}"
  echo "     userDN : $USER_DN"
...
echo "[OK] REVOKED: ${USER_UID} -> ${APP_CN}"
echo "     userDN : $USER_DN"
echo "     groupDN: $GROUP_DN"
